...
const Input: React.FC<InputProps> = ({ id, label, type, disabled, register, required, errors }) => {
  return (
    <div className='w-full relative'>
        <input {...register(id, {required})} placeholder='' type={type} autoComplete='off' disabled={disabled} id={id} className={`peer w-full p-4 pt-6 outline-none bg-white font-light border-2 rounded-md transition disabled:opacity-70 disabled:cursor-not-allowed ${errors[id] ? 'border-rose-400 focus:border-rose-400' : 'border-slate-300 focus:border-slate-300'}`}/>
